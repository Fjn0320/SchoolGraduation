<template>
    <div>
       <div style="margin: 10px 0">
           <el-input v-model="goname" style="width: 200px" placeholder="请输入名称" suffix-icon="el-icon-search"></el-input>
          ...
          <el-table 
           ...
           <el-table-column prop="dingdanid" label="订单序号" >
          </el-table-column>
...
export default {
       ...
       data() {
           return{
               ...
               username:localStorage.getItem("honey-user")?JSON.parse(localStorage.getItem("honey-user")).username:"",//该商家的id
               goname:"",
               tableData:[], 
               total:0,
               pageNum:1,
       ...
       methods: {
        showPingFen(){
           ...
           request.get("/ping/getPageAllPingShop",{
             params: {
                   pageNum: this.pageNum,
                   pageSize: this.pageSize,
                   goname:this.goname,//下单用户
         ...
         openDeletePing(id){//打开删除对话框
           ...
           deletePing(){
           request.post("/ping/pingDeleteShop/"+this.deleteId).then(res => {
                 console.log("删除的数据");
                 console.log(res);
                 if(res){
