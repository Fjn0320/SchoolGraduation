<template>
    <div>
       <div style="margin: 10px 0">
           <el-input v-model="shoping" style="width: 200px" placeholder="请输入名称" suffix-icon="el-icon-search"></el-input>
           <el-button @click="selectSort" class="ml-5" type="primary">搜索</el-button>
          ...
          <el-table 
           ...
           <el-table-column prop="shopsot" label="商品分类" >
           </el-table-column>
           <el-table-column prop="shoping" label="店名" >
           </el-table-column>
...
export default {
       ...
       methods: {
        showSort(){
           ...
           request.get("/sort/getPageAllSort",{
             params: {
                   pageNum: this.pageNum,
                   pageSize: this.pageSize,
                   shoping:this.shoping
                   }
           }).then(res => {
             console.log("获取的列表");
                   console.log(res);
         ...
         openDeletePing(id){//打开删除对话框
           ...
           deletePing(){
           request.post("/sort/sortDelete/"+this.deleteId).then(res => {
                 console.log("删除的数据");
                 console.log(res);
                 if(res){
         ...
         selectSort(){
             console.log("搜索的名字："+this.shoping);
             this.showSort()
           },
         rest(){//搜索栏重置
             this.shoping = ''
